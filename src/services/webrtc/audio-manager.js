...
class AudioManager {
  ...
  // react-native-webrtc's track.stop() only flips JS state (enabled=false,
  // readyState='ended'); the native stream/track/source stay in the module's
  // maps until the library-specific release() runs. Whoever drops a capture
  // here has to release it, or it leaks for the process lifetime.
  releaseInputStream() {
  ...
  async _mediaFactory(constraints = { audio: true }) {
    ...
    // The cached capture is dead - free it before acquiring a new one; a room
    // disconnect (forced reconnect, fatal-error reconnect, final teardown) ends
    // it in JS only.
  ...
  _setSenderTrackEnabled(shouldEnable) {
    ...
    // Redux audio.isMuted is the persisted rejoin intent (use-audio-join), and a
    // server mute held while the room reconnects leaves the bridge's intent alone -
    // mirroring the requested value would rejoin a deliberately-unmuted user muted.
  ...
  onAudioConnected(bridge) {
    // A bridge whose join resolved after exitAudio (or after a newer bridge
    // took over) must not flag audio connected: this.bridge is already null or
    // newer, so audio controls would be driving a dead bridge.
