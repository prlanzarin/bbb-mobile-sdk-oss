...
// A breakout mounts a second instance of this app against a single hoisted
// livekit-client, so a module-scoped guard would let both instances wrap
// methodFactory and forward every SDK line twice. Lines from the breakout
// instance are attributed to whichever instance installed the extension first.
const INSTALL_FLAG = '__bbbLiveKitSdkLogBridgeInstalled';
...
export const installLiveKitSdkLogBridge = (): void => {
  ...
  setLogExtension((level, msg, context) => {
    try {
      ...
      // Losing the line beats throwing back into the SDK's logging path. Release
      // builds strip the console, so there is no second sink to fall back to.
    }
  });
};
