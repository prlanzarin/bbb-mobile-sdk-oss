...
const BBBLiveKitRoom = ({ children }) => {
  ...
  // A camera teardown announced while a persistent notice owns the bar's single
  // slot waits for that slot instead of being clobbered by the re-assert below.
  // The profile is captured at announce time: 'cameraStoppedByLock' must not
  // degrade to 'cameraStopped' because the lock was lifted while it waited.
  ...
  const forceRoomReconnect = useCallback(({ source, resetAudio, resetCamera }) => {
    ...
    // connect effect re-joins audio only, and the camera is published on a
    // button press or an app resume. The server's camera row survives an
    // app-driven reconnect, so the row observer never sees this one - hence the
    // marking, so it does not announce it a second time if the row does go.
  ...
  // it over for good: assert and re-assert while the condition holds and the
  // notice has not been dismissed - a dismissed one leaves the slot free for
  // whatever is pending below it. One effect for all of them, in priority
  // order - separate ones would ping-pong the slot whenever a stall ends in a
  // give-up.
  useEffect(() => {
    const failedPending = reconnectNotice.current.room || reconnectNotice.current.fatal;
    // mediaReconnecting is gated on the room budget as well: a loop that has
    // given up is not "restoring your audio and video", so it must not inherit
    // the slot from a dismissed give-up notice.
